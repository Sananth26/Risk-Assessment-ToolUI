...
export default class  controlCustom extends control {
   ...
   */
  static getRegistered(type=false) : any {
    if (type) {
  ...
  build() {
    ...
    if (!custom) {
      return control.error('Invalid custom control type. Please ensure you have registered it correctly as a template option.');
    }

    // render the custom template
    // restore fieldData config structure for backwards compatibility
    let fieldData = (<any>Object).assign(this.config);
