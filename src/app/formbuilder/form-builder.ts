import '../../../node_modules/babel-generator';
...
export class FormBuilderCreateor {
  ...
  getFormBuilder(opts: any, element: any) {
      ...
      return   this.FormBuilder(opts,element);
    }

      FormBuilder =  (opts, element) =>{
      ...
      let $stage = (<any>$(d.stage));
        ...
        let prepFieldVars = function ($field, isNew = false) {
            ...
            if ($field instanceof jQuery) {
                ...
                if (field.type) {
                    ...
                    if (!isNew) {
                        field.values = (<any>$field).children().map((index, elem) => {
                            return {
                                label: $(elem).text(),
        ...
        const btnStyles = style => {
            ...
            styles.btn.forEach(btnStyle => {
                ...
                let btn = m('button', I18N.get(`styles.btn.${btnStyle}`), {
                    value: btnStyle,
                    type: 'button',
        ...
        const selectAttribute = (attribute, values, optionData) => {
            let selectOptions: any = optionData.map((option, i) => {
        ...
        $stage.on('click touchstart', '.remove', e => {
            let _event : any = e;
          let $field = jQuery(_event.target).parents('.form-field:eq(0)');
            let field = $field[0];
            let type: any = field.getAttribute('type');
        ...
        $stage.on('change', stageOnChangeSelectors.join(', '), e => {
            ...
            let field = utils.closest(_event.target, '.form-field');
            ...
            if (utils.inArray(field.type, optionTypes)) {
                ...
                let fieldVal: any = document.getElementById('value-' + field.id);
                if (fieldVal) {
        ...
        $stage.on('change', '.fld-multiple', e => {
            let _event: any = e;
            let newType : any = _event.target.checked ? 'checkbox' : 'radio';
            let $options : any = $('.option-selected', $(e.target).closest('.form-elements'));
      ...
      $stage.on('click', 'input.fld-access', function (e) {
        ...
        (<any>formBuilder).actions = {
            ...
            setLang: async locale => {
                ...
                d.empty(element);
                let formBuilder = this.FormBuilder(originalOpts, element);
