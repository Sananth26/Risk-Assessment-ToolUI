...
@Component({
  selector: 'app-viewsummaryuser',
  templateUrl: './viewsummaryuser.component.html',
  styleUrls: ['./viewsummaryuser.component.scss']  ,
  providers: [DatePipe, ViewfilesComponent]
})

export class ViewsummaryuserComponent implements OnInit {
...
loadfirewallrequest(id){
  ...
  let temp= JSON.parse(data.resultOP.riskandRankDetails)
  this.categoryLst =temp
...
loadAuditHistory(id){
  this.api.apiMethodFetchDataByGET("api/Audit/GetRequestData?requestid=" + id).then(result => {
    if (result.status) {
      ...
      console.log("Audit Trial+++++++++",this.auditTrial)
    } else {
      swal(
        'Error!',
        result.description,
