...
userId:any;
  ngOnInit() {
    console.log(localStorage.getItem("token")==null,localStorage.getItem("token"),"Token+++++++++")
    if(localStorage.getItem("token")!=null ){
      this.route.queryParams.subscribe(params=>{
        this.editRequestId =  params['requestId'];
        this.userId=params['userId'];
        console.log("Demooooo",this.userId)
        console.log("Demooooo",this.api.decode(localStorage.getItem("userId")))
       if(this.userId==this.api.decode(localStorage.getItem("userId"))){
        localStorage.setItem("redirectTo",'')  ;  
        this.onSummaryLoad();
        console.log("if")
         console.log(true,"if _____________________")
        }else{
         console.log("else part________")
         localStorage.setItem("redirectTo",'')
         /*swal({
  ...
  onSummaryLoad(){
    ...
    console.log('Summary Data',localStorage.getItem("primaryRequestId"))
  ...
  publish(){
    this.spinnerService.show()
    console.log('Publish Data',localStorage.getItem("primaryRequestId"))
  ...
  openModal() {
    this.spinnerService.show()
    console.log('Peer Data',localStorage.getItem("primaryRequestId"))
    const id=localStorage.getItem("primaryRequestId")
    const dialogConfig = new MatDialogConfig();
    dialogConfig.disableClose = true;
    dialogConfig.position= {top: '3%'}
  ...
  {this.spinnerService.show();
    this.api.apiMethodFetchDataByGET("api/Myrequest/Myrequest?status=" + status).then(result => {
      if (result.status) {
        ...
        this.spinnerService.hide();
         console.log(this.reviewrequestList);
      } else {
        swal(
          'Error!',
          result.description,
  ...
  navigate(){
   let id=this.editRequestId
    ...
    setTimeout(() => {
      ...
      this.router.navigate( ['urs/userstepper'], { queryParams: { requestId:id,view:'user'}})
...
loadfirewallrequest(id){
  ...
  if(data.resultOP.status=="Peer Review"){
    this.status=true;
  }
  this.LoadStepper(id)
  ...
  this.businessImpact    = data.resultOP.businessImpact
   ...
   console.log(JSON.parse(data.resultOP.securityPolicy.protocal))
   })
}
// File Upload
